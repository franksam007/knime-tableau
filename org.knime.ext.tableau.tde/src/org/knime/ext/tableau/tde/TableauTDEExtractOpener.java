 ...
 */
public class TableauTDEExtractOpener implements TableauExtractOpener {

    @Override
    public TableauExtract openExtract(final String path) throws WrappingTableauException {
        return new TableauTDEExtract(path);
    }

    private static class TableauTDEExtract implements TableauExtract {

        private final Extract m_extract;

        TableauTDEExtract(final String path) throws WrappingTableauException {
            try {
        ...
        public TableauTable createTable(final String name, final DataTableSpec spec) throws WrappingTableauException {
            try {
                ...
                // Create the table
                return new TableauTDETable(m_extract.addTable(name, tableDef), typeSetters);
            } catch (final TableauException e) {
                throw new WrappingTableauException(e);
            }
        }
        ...
        public TableauTable openTable(final String name, final DataTableSpec spec) throws WrappingTableauException {
            try {
                ...
                // Create the table
                return new TableauTDETable(table, typeSetters);
            } catch (final TableauException e) {
                throw new WrappingTableauException(e);
            }
        }
    ...
    private static class TableauTDETable implements TableauTable {

        private final Table m_table;

        private final TableauTypeSetter[] m_typeSetters;

        public TableauTDETable(final Table table, final TableauTypeSetter[] typeSetters) {
            m_table = table;
